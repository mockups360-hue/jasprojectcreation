...
const ProductCard = ({ id, name, price, originalPrice, image, discount, soldOut }: ProductCardProps) => {
  return (
    <div className="group relative">
      <Link to={`/product/${id}`} className="block">
        <div className="relative aspect-[3/4] rounded-lg overflow-hidden bg-secondary mb-3">
          ...
          {!soldOut && (
            <button 
              className="absolute bottom-3 right-3 bg-charcoal text-primary-foreground p-3 rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:scale-110 duration-300"
